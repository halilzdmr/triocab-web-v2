...
import { MapPin, Calendar, Clock, Users, Phone, Plane, Car, Navigation } from 'lucide-react';
...
/**
 * SharedTransferPage displays a transfer record accessed via a shareable link
 * Does not show pricing information and validates the token on load
 * 
 * Applying rule: Always add debug logs & comments in the code for easier debug & readability
 */
const SharedTransferPage: React.FC = () => {
  ...
  // State to track which location's map menu is open (pickup or dropoff)
  // Applying rule: Always add debug logs & comments in the code for easier debug & readability
  ...
  useEffect(() => {
    const fetchTransfer = async () => {
      try {
        ...
        console.debug('Fetching shared transfer with token:', token);
        
        if (!token) {
          console.error('No token provided in URL');
          setError('Invalid or missing token. This link may have expired or been tampered with.');
          setLoading(false);
          return;
        }
        ...
        const response = await fetch(`${API_BASE_URL}/transfers/share?token=${token}`);
        
        if (!response.ok) {
          const errorData = await response.json();
          console.error('Error fetching shared transfer:', errorData);
          
          if (response.status === 401) {
            setError('This link has expired. Please contact the transfer provider for a new link.');
          } else if (response.status === 404) {
            setError('Invalid link. This transfer record cannot be found.');
          } else {
            setError(errorData.message || 'An error occurred while retrieving the transfer details.');
          }
        ...
        // More comprehensive debug logging to identify available location fields
        // Applying rule: Always add debug logs & comments in the code for easier debug & readability
        console.debug('Shared transfer data received:', data);
        
        // Log all field names from the transfer object to help with troubleshooting
        console.debug('Available transfer fields:', Object.keys(data.transfer));
        
        // Log specific location fields with their values
        console.debug('Location field values:', {
          // Check both potential field name formats (camelCase and PascalCase)
          pickupLatitude: data.transfer.Pickup_Location__Latitude__s,
          pickupLongitude: data.transfer.Pickup_Location__Longitude__s,
          dropoffLatitude: data.transfer.Dropoff_Location__Latitude__s,
          dropoffLongitude: data.transfer.Dropoff_Location__Longitude__s
        });
        
        // Convert Salesforce data to frontend Transfer model
        const transferData: Transfer = {
          id: data.transfer.Id,
          reservationId: data.transfer.Name,
          longPickupAddress: data.transfer.Pickup_Address__c || '',
          longDropoffAddress: data.transfer.Dropoff_Address__c || '',
          bookingReference: data.transfer.Name || '',
          passengerName: data.transfer.Passenger_Name__c || '',
          passengerCount: data.transfer.Passenger_Count__c || 1,
          contactPhone: data.transfer.Passenger_Telephone_Number__c || '',
          // These fields were removed from the Salesforce query as they don't exist
          // Applying rule: Always add debug logs & comments in the code for easier debug & readability
          origin: '',  // No longer available from the API
          destination: '', // No longer available from the API
          date: new Date(data.transfer.Pickup_Date_Time__c).toLocaleDateString('en-GB', {
            year: 'numeric',
            month: 'short',
            day: 'numeric'
          }),
          pickupTime: new Date(data.transfer.Pickup_Date_Time__c).toLocaleTimeString('en-GB', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: false
          }),
          flightNumber: data.transfer.Flight_Number__c || '',
          vehicleType: data.transfer.Vehicle_Type__c || '',
          status: data.transfer.Journey_Status__c?.toLowerCase() || 'pending',
          createdAt: new Date().toISOString(),
          // Add location data if available
          // Applying rule: Always add debug logs & comments in the code for easier debug & readability
          // Debug log all location fields to help troubleshoot
          pickupLocation: (() => {
            console.debug('Pickup location data:', {
              lat: data.transfer.Pickup_Location__Latitude__s,
              lng: data.transfer.Pickup_Location__Longitude__s,
              rawFields: Object.keys(data.transfer).filter(key => key.includes('Location'))
            });
            
            return data.transfer.Pickup_Location__Latitude__s && data.transfer.Pickup_Location__Longitude__s ? {
              latitude: data.transfer.Pickup_Location__Latitude__s,
              longitude: data.transfer.Pickup_Location__Longitude__s
            } : undefined;
          })(),
          
          dropoffLocation: (() => {
            console.debug('Dropoff location data:', {
              lat: data.transfer.Dropoff_Location__Latitude__s,
              lng: data.transfer.Dropoff_Location__Longitude__s,
              rawFields: Object.keys(data.transfer).filter(key => key.includes('Location'))
            });
            
            return data.transfer.Dropoff_Location__Latitude__s && data.transfer.Dropoff_Location__Longitude__s ? {
              latitude: data.transfer.Dropoff_Location__Latitude__s,
              longitude: data.transfer.Dropoff_Location__Longitude__s
            } : undefined;
          })()
        };
        ...
        console.error('Error in fetchTransfer:', error);
  ...
  const getDirectionsUrl = (location: { latitude: number; longitude: number } | undefined, address: string, mapService: 'google' | 'apple' | 'yandex' = 'google'): string => {
    ...
    if (mapService === 'google') {
      if (location) {
        return `https://maps.google.com/maps?daddr=${location.latitude},${location.longitude}`;
      } else {
      ...
      if (location) {
        ...
        return `maps://?daddr=${location.latitude},${location.longitude}`;
      } else {
        return `maps://?daddr=${encodeURIComponent(address)}`;
      }
    } else if (mapService === 'yandex') {
      if (location) {
        return `https://yandex.com/maps/?rtext=~${location.latitude},${location.longitude}`;
      } else {
  ...
  // Map selection bottom sheet component (native-style)
    ...
    return (
      <div className="fixed inset-0 z-[9999] isolate">
        ...
        >
          ...
          <h3 className="text-lg font-semibold text-center mb-4">Open with Map</h3>

          <div className="grid grid-cols-3 gap-4 pb-6">
            {(['google', 'apple', 'yandex'] as const).map(service => (
              <a
                key={service}
                href={getDirectionsUrl(location, address, service)}
                className="flex flex-col items-center justify-center p-3 rounded-lg transition-colors hover:bg-neutral-50"
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => {
                  console.debug(`Opening ${service} maps`);
                  setOpenMapMenu(null);
                }}
              >
                {service === 'google' && <div><img 
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-50 p-4">
        ...
        <p className="mt-4 text-neutral-600">Loading transfer details...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-50 p-4">
        <div className="bg-white rounded-xl shadow-md p-6 max-w-md w-full">
          <div className="flex flex-col items-center text-center">
            <div className="flex justify-center items-center h-12 w-12 rounded-full bg-red-100 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <h2 className="text-xl font-bold text-neutral-900 mb-2">Unable to Load Transfer</h2>
            <p className="text-neutral-600 mb-6">{error}</p>
            <button
              onClick={() => navigate('/')}
              className="bg-primary-600 hover:bg-primary-700 text-white rounded-lg py-2 px-4 transition-colors"
            >
              Go to Dashboard
            </button>
          </div>
  ...
  if (!transfer) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-neutral-50 p-4">
        <div className="bg-white rounded-xl shadow-md p-6 max-w-md w-full">
          <div className="flex flex-col items-center text-center">
            <div className="flex justify-center items-center h-12 w-12 rounded-full bg-yellow-100 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <h2 className="text-xl font-bold text-neutral-900 mb-2">No Transfer Found</h2>
            <p className="text-neutral-600 mb-6">The requested transfer could not be found or the link has expired.</p>
            <button
              onClick={() => navigate('/')}
              className="bg-primary-600 hover:bg-primary-700 text-white rounded-lg py-2 px-4 transition-colors"
            >
        ...
        <div className="max-w-2xl mx-auto p-4">
          <div className="bg-white rounded-xl shadow-md p-6 my-8">
            <div className="flex items-center justify-between mb-6">
              <h1 className="text-xl font-bold text-neutral-900">Transfer Details</h1>
            ...
            <div className="mb-6">
              ...
              <div className="text-lg font-medium text-neutral-900">{transfer.bookingReference}</div>
            </div>
                ...
                <div className="flex">
                  ...
                  <div>
                    ...
                    <div className="flex items-start">
                      <div className="text-neutral-900">{transfer.longPickupAddress}</div>
                      {transfer.pickupLocation && (
                        <button
                          type="button"
                          onClick={(e) => {
                            e.preventDefault();
                            setOpenMapMenu(prev => prev === 'pickup' ? null : 'pickup');
                            console.debug('Toggle map menu for pickup location');
                          }}
                          className="text-primary-600 ml-2"
                          aria-label="Open map options for pickup location"
                        >
                          <Navigation size={18} />
                        </button>
                      )}
                ...
                {transfer.longDropoffAddress && (
                  <div className="flex">
                    ...
                    <div>
                      ...
                      <div className="flex items-start">
                        <div className="text-neutral-900">{transfer.longDropoffAddress}</div>
                        {transfer.dropoffLocation && (
                          <button
                            type="button"
                            onClick={(e) => {
                              e.preventDefault();
                              setOpenMapMenu(prev => prev === 'dropoff' ? null : 'dropoff');
                              console.debug('Toggle map menu for dropoff location');
                            }}
                            className="text-primary-600 ml-2"
                            aria-label="Open map options for dropoff location"
                          >
                            <Navigation size={18} />
                          </button>
                        )}
...
export default SharedTransferPage;
